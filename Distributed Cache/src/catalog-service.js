...
function log(message) {
  ...
  console.log(`[Catalog Service]        [T+${elapsed}s] ${message}`);
}
...
async function performCheck() {
  ...
  if (cachedData) {
    ...
    log(`Check Redis cache for "${PRODUCT_KEY}" -> [CACHE MISS] (took ${latency}ms)`);
...
async function run() {
  // Perform first check immediately at T = 0s
  await performCheck();

  // Perform secondary check at absolute T = 18s
  setTimeout(async () => {
    await performCheck();
    redis.quit();
